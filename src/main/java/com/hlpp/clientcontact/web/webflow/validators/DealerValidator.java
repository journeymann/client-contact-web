...
public class DealerValidator {
     ...
     * Validates 'dealerForm' view state after binding to contact.
     */
    public void validateDealerForm(BrokerDealer dealer, MessageContext context) {

        if (!StringUtils.hasText(dealer.getBrokerDealerName())) {
            context.addMessage(new MessageBuilder().error().source("brokerDealerName").code("dealer.form.brokerDealerName.error").build());
        }
